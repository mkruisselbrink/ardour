...
static std::string
stringify_descriptor (CAComponentDescription const& desc)
{
...
auv2_discover (ArdourComponent& comp, CAComponentDescription& desc, std::vector<ARDOUR::AUv2Info>& rv, bool verbose)
{
	...
	info.id = stringify_descriptor (desc);
...
{
	std::string id = stringify_descriptor (desc);
	char hash[41];
	Sha1Digest s;
	sha1_init (&s);
...
string
ARDOUR::auv2_valid_cache_file (CAComponentDescription const& desc, bool verbose)
{
...
ARDOUR::auv2_scan_and_cache (CAComponentDescription& desc, boost::function<void (CAComponentDescription const&, AUv2Info const&)> cb, bool verbose)
{
	...
	try {
		...
		if (nfo.empty ()) {
			cerr << "No plugins matching ID: '" << stringify_descriptor (desc) << "'\n";
			delete root;
			return false;
		}
		...
		cerr << "Cannot load AudioUnit plugin: '" << stringify_descriptor (desc) << "'\n";
		delete root;
		return false;
	}
